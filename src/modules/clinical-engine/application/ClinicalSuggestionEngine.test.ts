...
function makePatient(
  overrides: Partial<Parameters<typeof Patient.reconstitute>[0]> = {},
): Patient {
...
function makeInputs(
  overrides: Partial<SuggestionInputs> = {},
): SuggestionInputs {
  return {
    patient: makePatient(),
    anthropometry: null,
...
describe("ClinicalSuggestionEngine.suggestDiagnoses", () => {
  ...
  it("detecta obesidad grado 1, 2 y 3 seg\u00fan IMC", () => {
    expect(
      engine
        .suggestDiagnoses(
          makeInputs({ anthropometry: makeAnthropometry(88, 170) }),
        )
        .some((s) => s.code === "obesidad_grado_1"),
    ).toBe(true);
    expect(
      engine
        .suggestDiagnoses(
          makeInputs({ anthropometry: makeAnthropometry(105, 170) }),
        )
        .some((s) => s.code === "obesidad_grado_2"),
    ).toBe(true);
    expect(
      engine
        .suggestDiagnoses(
          makeInputs({ anthropometry: makeAnthropometry(120, 170) }),
        )
        .some((s) => s.code === "obesidad_grado_3"),
    ).toBe(true);
  });
  ...
  it("detecta hipercolesterolemia", () => {
    const inputs = makeInputs({
      labPanel: makeLabPanel({ COLESTEROL_TOTAL: 250 }),
    });
    const result = engine.suggestDiagnoses(inputs);
  ...
  it("detecta hipertrigliceridemia", () => {
    const inputs = makeInputs({
      labPanel: makeLabPanel({ TRIGLICERIDOS: 220 }),
    });
    const result = engine.suggestDiagnoses(inputs);
  ...
  it("detecta anemia seg\u00fan sexo del paciente", () => {
    ...
    expect(
      engine.suggestDiagnoses(femaleInputs).some((s) => s.code === "anemia"),
    ).toBe(true);
    ...
    expect(
      engine.suggestDiagnoses(maleInputs).some((s) => s.code === "anemia"),
    ).toBe(true);
  });
  ...
  it("detecta enfermedad renal cr\u00f3nica con creatinina elevada (eGFR < 60)", () => {
    ...
    const result = engine.suggestDiagnoses(inputs);
    expect(result.some((s) => s.code === "enfermedad_renal_cronica")).toBe(
      true,
    );
  });
  ...
  it("detecta sospecha de h\u00edgado graso con enzimas elevadas + sobrepeso", () => {
    ...
    const result = engine.suggestDiagnoses(inputs);
    expect(result.some((s) => s.code === "higado_graso_no_alcoholico")).toBe(
      true,
    );
  });
  ...
  it("detecta disfunci\u00f3n tiroidea con TSH alterada", () => {
    ...
    expect(
      engine.suggestDiagnoses(high).some((s) => s.code === "tiroideo"),
    ).toBe(true);
    const low = makeInputs({ labPanel: makeLabPanel({ TSH: 0.2 }) });
    expect(
      engine.suggestDiagnoses(low).some((s) => s.code === "tiroideo"),
    ).toBe(true);
  });
  ...
  it("detecta riesgo cardiometab\u00f3lico por RCC elevada seg\u00fan sexo", () => {
    ...
    expect(
      engine
        .suggestDiagnoses(femaleInputs)
        .some((s) => s.code === "sindrome_metabolico"),
    ).toBe(true);
  });
...
describe("ClinicalSuggestionEngine.suggestMealPlanTargets", () => {
  ...
  it("la distribuci\u00f3n suma 100%", () => {
    ...
    const sum =
      result.distribution.carbsPct +
      result.distribution.proteinPct +
      result.distribution.fatPct;
    expect(sum).toBe(100);
  });
