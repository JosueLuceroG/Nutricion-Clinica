...
test.describe("Meal Planner & Chef AI", () => {
  ...
  test("meal planner page shows the empty state when no plans exist", async ({ page }) => {
    await page.goto("/#/plan-semanal");
    await expect(page.locator("h1")).toContainText(/planificador/i, { timeout: 10_000 });
    await expect(page.getByText(/sin planes|no hay|no weekly plans|sin planes/i)).toBeVisible({
      timeout: 5_000,
    });
  });
  ...
  test("chef dialog opens and shows the form", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /cocinar con ia|ai chef|chef/i }).first().click();
