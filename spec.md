...
- **Sprint actual:** Sprint 39 completado — soporte TURN configurable para WebRTC (`VITE_STUN_URLS`, `VITE_TURN_URLS`, `VITE_TURN_USERNAME`, `VITE_TURN_CREDENTIAL`), fix de ICE candidates con `salaId`, fix de arranque de signaling con el `localStream` recién obtenido, y grabación local de sesiones con `MediaRecorder` descargando `.webm`. Siguiente slice recomendado: consentimiento explícito de grabación + almacenamiento cifrado de grabaciones.
