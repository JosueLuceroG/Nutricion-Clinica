...
- **Sprint actual:** Sprint 38 completado — signaling server WebSocket (`ws`) en `/ws/telemedicina` con autenticación JWT y relay de ofertas/answers/ICE candidates entre peers en una sala; hook `useWebRTC` con `RTCPeerConnection` + STUN de Google; `VideoCallRoom` actualizado para peer-to-peer real. CI/CD ya existente en `.github/workflows/ci.yml`. Siguiente slice recomendado: TURN server para peers detrás de NAT restrictivo, o grabación de sesiones.
